...
class Unit extends AUTH_Controller {
	...
	public function index() {
		...
		$this->template->views('unit/home', $data);
	}

	public function tampil() {
		$data['dataUnit'] = $this->M_unit->select_all();
		$this->load->view('unit/list_data', $data);
	}
	...
	public function update() {
		$data['userdata'] 	= $this->userdata;
		$id 				= trim($_POST['kd_unit']);
		$data['dataUnit'] 	= $this->M_unit->select_by_id($id);

		echo show_my_modal('modals/modal_update_unit', 'update-unit', $data);
	}
	...
	public function prosesUpdate() {
		...
		$data 	= $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_unit->update($data);
			...
			if ($result > 0) {
				...
				$out['status'] = '';
				$out['msg'] = show_err_msg('Data Unit Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());

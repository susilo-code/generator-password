	...
	public function prosesTambah() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				...
				$config['smtp_user'] = 'takdir@stis.ac.id';
				$config['smtp_pass'] = '';

				$this->load->library('email', $config);
				$this->email->set_newline("rn");

				$this->email->from('takdir@stis.ac.id', 'Takdir');
				$this->email->to('takdir@stis.ac.id');

				$this->email->subject('Penambahan Pegawai Baru');
				$this->email->message('Pegawai baru telah ditambahkan.');
